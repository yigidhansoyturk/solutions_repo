...
// Populate node selectors after DOM and vis network setup
setTimeout(() => {
...
def simplify(G, start, end):
  ...
  <section>
    ...
    <p>This application already includes several advanced features to support deeper analysis and interactivity:</p>
<ul>
  <li><strong>Dynamic drag-and-drop editor:</strong> Easily reposition nodes and restructure the circuit visually using the interactive canvas.</li>
  <li><strong>Custom node selection:</strong> Compute the equivalent resistance between any pair of nodes using the dropdowns.</li>
  <li><strong>Visual resistance graph:</strong> View real-time circuit configurations with labeled resistances and adjust them on the fly.</li>
</ul>
<p>Upcoming additions will enhance export capabilities, support symbolic resistance simplification, and enable animated current/voltage simulations in real-time.</p>
  </section>
<script>
function exportNetworkImage() {
  html2canvas(document.getElementById('network')).then(canvas => {
    const link = document.createElement('a');
    link.download = 'circuit-network.png';
...
function exportNetworkJSON() {
  ...
  link.setAttribute("download", "circuit-network.json");
